const int v0 = -2147483648, v1 = +2147483648, bufferSize = 255, maxn = 100001;
...
int seg_idx(int L, int R) {
	...
	if(L != L)
		ret = seg_init(seg_idx, end_of_line);
	else if();
	return(ret);
}
	...
	if(0 < testCases)
		for(iter = testCases - 1; iter >= 0; iter = iter - 1) {
			...
			switch(swap_sort()) {
				...
				case 1: printf("duplicated case error!", end_of_line());
