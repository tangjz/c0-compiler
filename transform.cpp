#ifdef GLOBAL_REGISTER_ALLOCATION
#include "transfrom.h"
...
const int $zero = 0, $v0 = 2, $a0 = 4, $t8 = 24, $t9 = 25, $gp = 28, $sp = 29, $ra = 31;
const int localRegLow = 8, localRegUpp = 23;
...
void printRegImm(const char *op, int rt, int rs, int offset) { // addi, subi
    if(offset > OFFSET_MAX) {
        printRegImm("li", $t8, offset);
        printRegReg(op, rt, rs, $t8);
...
void printRegMem(const char *op, int rt, int rs, int offset) { // lb, lw, sb, sw
    ...
    if(offset > OFFSET_MAX) {
        printRegImm("li", $t8, offset);
        printRegReg("add", $t8, rs, $t8);
        fprintf(fout, "\t" "%s %s, 0(%s)" "\n", op, regName[rt], regName[$t8]);
    } else {
...
char refToken[REG_MAX][TOKEN_MAX];

int findReg(const char *token) {
    for(int id = localRegLow; id <= localRegUpp; ++id)
        if(strcmp(refToken[id], token) == 0) {
            refCnt[refToken[id]] += effects;
            return id;
        }
    return -1;
}
void setRegToMem(int id) {
    if(id >= localRegLow && id <= localRegUpp) {
        assert(strcmp(refToken[id], "") != 0);
        int index = findSymbol(refToken[id]);
        if(index == -1) { // temporary variable
            printRegMem("sw", id, $sp, -getTemporaryIndex(refToken[id]) * BYTE_PER_INT);
        } else {
            printRegMem(symbolTable[index].type == INT ? "sw" : "sb", id, symbolTable[index].isGlobal ? $gp : $sp, symbolOffset[index]);
        }
    }
}
void getMemToReg(int id) {
    if(id >= localRegLow && id <= localRegUpp) {
        assert(strcmp(refToken[id], "") != 0);
        int index = findSymbol(refToken[id]);
        if(index == -1) { // temporary variable
            printRegMem("lw", id, $sp, -getTemporaryIndex(refToken[id]) * BYTE_PER_INT);
        } else {
            printRegMem(symbolTable[index].type == INT ? "lw" : "lb", id, symbolTable[index].isGlobal ? $gp : $sp, symbolOffset[index]);
        }
    }
}
int getReg(const char *token) {
    int lowRef = INT_MAX, lowIdx = -1;
    for(int id = localRegLow; id <= localRegUpp; ++id)
        if(strcmp(refToken[id], "") == 0) {
            strcpy(refToken[id], token);
            return id;
        } else if(refCnt[refToken[id]] < lowRef) {
            lowRef = refCnt[refToken[id]];
            lowIdx = id;
        }
    setRegToMem(lowIdx);
    return lowIdx;
...
void convertMIPS() {
    fprintf(fout, "#Copyright: Jingzhe Tang, Jan. 16th, 2018\n");
    ...
    for(int codeIndex = 0, functionIndex = -1; codeIndex < codeCount; ++codeIndex) {
        ...
        if(strncmp(cur.op, "def", 3) == 0 || strcmp(cur.op, "pushPara") == 0)
            continue;
        int index = -1, offset = 0, $rs = -1, $rt = -1, $rd = -1;
        ...
        if(strcmp(cur.op, "add") == 0 || strcmp(cur.op, "sub") == 0) { // add/sub, left, right, destination
            ...
            if(isLetter(cur.rht[0]) && ((index = findSymbol(cur.rht)) == -1 || symbolTable[index].kind != CONST)) {
                ...
                if(symbolTable[arrayIndex].type == INT) {
                    printRegImm("li", $t8, BYTE_PER_INT);
                    printRegReg("mult", $rt, $t8);
                    printReg("mflo", $rt);
                }
